...
def full_comparer(newsp_A, newsp_B, date, hour, cosine=False):
    ...
    jsonizer(comparison, "./full_compared/ANSA_TEST/AGI_TEST/cos_15+-7.json")
    #print(VISUAL_FILE)
...
def multi_news_getter(newsp, start_hour, date):
    ...
    for subdir in os.scandir(f"{BASE_DIR}/{newsp}"):
        ...
        if scrape_date == dates[0]:
            if scrape_hour in range(start_hour, final_hour) or ("AGI" in newsp and scrape_hour in range(start_hour - 7, start_hour + 7)):
